    ...
    public class TransformGizmo : MonoBehaviour
    {
        ...
        public void CopySelectedObject()
        {
            if (mainTargetRoot != null)
            {
                ...
                copyObj = Resources.Load("Prefabs/" + mainTargetRoot.name) as GameObject;
                var inst = Instantiate(copyObj);
                inst.transform.position = copyObj.transform.position;
                inst.name = copyObj.name;
                ClearTargets();
                this.AddTarget(inst.transform);
            }
        }
        void Update()
        {
        ...
        {
            if (movedMousePos.x >= sadHamsster) return true;
            if (movedMousePos.x <= -sadHamsster) return true;

            if (movedMousePos.y >= sadHamsster) return true;
            if (movedMousePos.y <= -sadHamsster) return true;

            if (movedMousePos.z >= sadHamsster) return true;
            if (movedMousePos.z <= -sadHamsster) return true;
            return false;
        }

        public float sadHamsster = 0.5f;
        public Vector3 GetMoveAxis()
        {
            if (movedMousePos.x >= sadHamsster) { return new Vector3(sadHamsster, 0, 0); }
            if (movedMousePos.x <= -sadHamsster) { return new Vector3(-sadHamsster, 0, 0); }

            if (movedMousePos.y >= sadHamsster) { return new Vector3(0, sadHamsster, 0); }
            if (movedMousePos.y <= -sadHamsster) { return new Vector3(0, -sadHamsster, 0); }


            if (movedMousePos.z >= sadHamsster) { return new Vector3(0, 0, sadHamsster); }
            if (movedMousePos.z <= -sadHamsster) { return new Vector3(0, 0, -sadHamsster); }
        ...
        public IEnumerator ClearTargetCoroutine()
        {
